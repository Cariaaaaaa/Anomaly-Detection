...
def cvt2heatmap(gray):
    heatmap = cv2.applyColorMap(np.uint8(gray), cv2.COLOR_MAP_JET)
    return heatmap

def evaluation(encoder, bn, decoder, dataloader, device, auxiliary_classifier):
    print('Evaluating...')
    bn.eval()
    decoder.eval()
    ...
    with torch.no_grad():
        ...
        if auxiliary_classifier:
            ...
            print(f'Auxiliary Classifier Test Accuracy: {test_accuracy:.2f}%')
...
def test(_class_):
    ...
    print(device)
    print(_class_)
    data_transform, gt_transform = get_data_transforms(768, 768)
    test_path = './mvtec/' + _class_
    ckp_path = './checkpoints/' + 'wres50_' + _class_ + 'Best epoch' + '.pth'
...
def visualization(_class_):
    ...
    with torch.no_grad():
        for img, gt, label, _ in test_dataloader:
            ...
            anomaly_map[anomaly_map > 0.24] = 1  ##这里是ROI的阈值显示。经我观察 0.18-2.6之间为最佳
            ...
            plt.title('ROI',y=-0.3)
            plt.savefig(f'./save3_AANEWNEW/aaaaadddd{idx}.png')
            print('picture  '+str(idx) + '  saved')
            idx+=1
            # plt.show()
